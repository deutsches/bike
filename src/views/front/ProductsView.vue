<template>
  ...
  <div style="background-color: #fffcf7">
    <div class="container pt-5 pb-5">
      <div class="row">
        <div class="col-12 d-lg-inline-block col-lg-2">
          <LeftMenu @get-products="getProducts"></LeftMenu>
        </div>
        <div class="cols-12 col-lg-10">
          <div class="mb-4 position-relative">
            <input type="text" placeholder="在此搜尋車種" v-model="search" class="input rounded-pill">
          ...
          <div class="row row-cols-1 row-cols-lg-3 g-4">
            <div class="col rounded-5" v-for="item in tempArray" :key="item.id">
              <div class="card shadow">
                ...
                class="btn btn-blacl btn-outline-white border-top w-100"></AddCartButton>
                </div>
              </div>
            </div>
          </div>
    ...
    <PagnationComponent :page="page" :get-products="getProducts" @next-page="getProducts"
     :category="category">
    </PagnationComponent>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getProducts(page = 1, category = '全部') {
      let url = '';
